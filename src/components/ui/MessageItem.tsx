...
import { GitHubActivity } from "../ai-components/GitHubActivity";
...
const MessageItem: React.FC<MessageItemProps> = React.memo(
  ({
    ...
    const role =
      message.role === "user" ? "Me" : process.env.NEXT_PUBLIC_ASSISTANT_NAME;
        ...
        return (
          <>
            ...
            {part.toolInvocation.toolName === "getExperience" &&
              part.toolInvocation?.result?.experiences.length > 0 && (
                <div className="w-full">
                  <Experiences
                    experiences={part.toolInvocation?.args?.experiences}
                  />
                </div>
              )}
            ...
            {part.toolInvocation.toolName === "getLocation" && (
              <div className="w-full">
                <Locations
                  locations={part.toolInvocation?.result?.cities}
            ...
            {part.toolInvocation.toolName === "getPhotos" && (
              <div className="w-full">
                <PhotoGrid
                  photos={part.toolInvocation?.result?.photos}
            ...
            {part.toolInvocation.toolName === "getFootball" &&
              part.toolInvocation?.result?.matches.length > 0 && (
                <div className="w-full">
                  <Football
                    matches={part.toolInvocation?.result?.matches}
                    translations={translations}
                  />
                </div>
              )}
            {part.toolInvocation.toolName === "getGithub" &&
              part.toolInvocation?.result?.weeks && (
                <div className="w-full">
                  <GitHubActivity data={part.toolInvocation?.result} />
                </div>
              )}
          </>
        );
    ...
    return (
      <div
        className={`flex ${isFirst && "pb-2"} ${message.role === "user" ? "justify-end" : "justify-start"} items-end`}
      >
        <div className="flex flex-col w-full">
          <div
            className={`flex flex-col w-[100%] rounded-2xl p-2 ${message.role === "user" ? "bg-[#2457ff] text-white dark:bg-white/15" : "bg-gray-700/5 text-gray-700 dark:bg-white/5"} dark:text-foreground backdrop-blur-md markdown-body`}
          >
            ...
            {formattedTime && (
              <div className="flex w-full gap-2 justify-end font-doto font-bold text-xs mt-2">
                <span>{role}</span>
                <span className="text-[#2457ff] dark:text-yellow-500">
                  {formattedTime}
                </span>
              </div>
            )}
          ...
          {isFinished && message.role !== "user" && isLastAssistantMessage && (
            <div className="w-full mt-3 prompt-carousel">
              <PromptCarousel
                handleSubmitPrePrompt={handleSubmitPrePrompt}
                translations={translations}
              />
